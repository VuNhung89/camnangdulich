...
const postForm = useForm({ title: '', content: '', location: '', image: null });
...
const createPost = () => {
  ...
  const formData = postForm.data();
  console.log('Form data before submission:', formData); // Gỡ lỗi dữ liệu biểu mẫu
  if (!formData.image) {
    delete formData.image; // Xóa trường image nếu null
  }
  ...
  postForm.post('/api/posts', {
    ...
    onError: (err) => {
      ...
      errors.value.createPost = Object.values(err).join(', ') || 'Lỗi khi đăng bài';
...
const handlePostImageChange = (event) => {
  const file = event.target.files[0];
  if (!file) {
    postForm.image = null; // Đặt thành null nếu không chọn tệp
    errors.value.createPost = null; // Xóa lỗi nếu không chọn tệp
    return;
  }
  if (!file.type.startsWith('image/')) {
    postForm.image = null;
    errors.value.createPost = 'Tệp được chọn không phải là hình ảnh hợp lệ.';
    return;
  }
...
<template>
  <div class="min-h-screen bg-gradient-to-b from-blue-100 to-teal-100 p-1 animate-fade-in">
    <Header title="Hồ sơ cá nhân" class="mb-8"/>
    <div class="max-w-5xl mx-auto">
      ...
      <div v-else class="space-y-8">
        <!-- Header -->
        <div class=" bg-gradient-to-r from-blue-400 to-sky-600 p-8 rounded-2xl shadow-xl text-center transform hover:scale-105 transition-transform duration-300">
          <div class="relative inline-block">
            <img
              :src="user.avatar ? `${baseUrl}${user.avatar}` : 'https://via.placeholder.com/150'"
              alt="User Avatar"
            ...
            <div>
              ...
              <div class="flex flex-wrap gap-2 mb-4">
                <button
                  ...
                  class="bg-gray-400 text-white px-3 py-1 rounded hover:bg-blue-600"
                >
                  H1
                </button>
                <button
              ...
              <textarea
                ...
                class="w-full px-4 py-3 border border-teal-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-teal-500"
                rows="6"
                placeholder="Nhập nội dung, có thể dùng thẻ <h1> cho Tiêu đề lớn, <h2> cho Tiêu đề phụ, <p> cho đoạn văn,..."
              ></textarea>
