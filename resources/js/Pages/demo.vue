...
onMounted(async () => {
  ...
  try {
    ...
    posts.value = postsRes.data.map(post => ({
      ...post,
      likes_count: post.likes?.length || 0,
      comments_count: post.comments?.length || 0,
      average_rating: post.ratings?.length ? (post.ratings.reduce((sum, r) => sum + r.rating, 0) / post.ratings.length).toFixed(1) : 0,
    }));
    
    // Sắp xếp events theo date gần nhất
    events.value = eventsRes.data.sort((a, b) => new Date(a.date) - new Date(b.date));
            ...
            <div class="relative h-[70vh] w-full">
              ...
              <div class="absolute inset-0 bg-black/50 flex items-center justify-center">
                <div class="text-center text-white p-6">
                  ...
                  <a
                    href="/tours"
                    class="bg-yellow-600 hover:bg-blue-700 text-white px-8 py-3 rounded-full font-semibold transition duration-300"
                  >
                    Lên kế hoạch
                  </a>
                </div>
    ...
    <div class="bg-white p-6 rounded-lg shadow-md">
      ...
      <div class="flex flex-col sm:flex-row gap-4">
        ...
        <button
          ...
          class="bg-yellow-600 hover:bg-blue-700 text-white px-6 py-3 rounded-full font-semibold transition duration-300 disabled:bg-yellow-500 disabled:cursor-not-allowed"
        >
          Tìm kiếm
        </button>
      </div>
  ...
  <section v-if="searchResults?.locations?.length || searchResults?.tours?.length || searchResults?.hotels?.length || searchResults?.posts?.length" class="max-w-7xl mx-auto px-4">
          ...
          <Slide v-for="event in events.slice(0, 6)" :key="event.id">
            <div class="bg-white rounded-lg shadow-md overflow-hidden mx-2">
              ...
              <div class="p-4">
                ...
                <p class="text-gray-600 mt-2" v-html="truncateText(event.description, 100)"></p>
                <p class="text-sm text-gray-500 mt-2">Ngày: {{ formatDate(event.date) }}</p>
                <a
                  href="/events"
                  class="mt-4 inline-block text-yellow-600 hover:text-blue-700 font-semibold"
                >
                  Xem chi tiết
                </a>
              </div>
      ...
      <section class="max-w-7xl mx-auto px-4">
        ...
        <div class="text-center mt-6">
          <a
            href="/locations"
            class="bg-yellow-600 hover:bg-blue-700 text-white px-6 py-3 rounded-full font-semibold transition duration-300"
          >
            Xem tất cả
          </a>
        </div>
